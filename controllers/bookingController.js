...
const photographerModel = require("../models/photographerModel");
const roomModel = require("../models/roomModel");
const notificationModel = require("../models/notificationModel"); // Assuming this exists
const Razorpay = require("razorpay");

const instance = new Razorpay({
  key_id: process.env.RAZORPAY_KEY_ID || "rzp_test_6TnEsQ4TEWtn9r",
  key_secret: process.env.RAZORPAY_KEY_SECRET || "wODChxDTrIDPfHjISZMvERdd",
});

// Adds a new booking with payment integration
exports.addBooking = async (req, res) => {
  try {
    const {
      mandapId,
    ...
    if (paymentId) {
      ...
      if (
        ...
        return res
          .status(400)
          .json(createErrorResult("Invalid or failed payment"));
      }
    } else {
      // Create Razorpay order if payment not yet processed
      const options = {
      ...
      return res.status(200).json(createSuccessResult({ order }));
    }
